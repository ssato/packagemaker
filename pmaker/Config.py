...
def parse_args(argv=sys.argv[1:], defaults=None, upto=UPTO,
    ...
    bstep_choices = [n for n, _l, _h in build_steps]
    bog.add_option("", "--upto", type="choice", choices=bstep_choices,
        help = "Packaging step you want to proceed to: %s [%%default]" % \
            bstep_choices
    )
    bog.add_option("", "--format", type="choice", choices=formats,
        help="Package format: %s [%%default]" % ", ".join(choices),
    )
    bog.add_option("", "--driver", type="choice",
        choices=unique(drivers.keys()),
        help="Packaging driver: %s [%%default]" % ", ".join(drivers),
    )
    bog.add_option("", "--itype", type="choice", choices=itypes.keys(),
...
class Config(object):
    ...
    def defaults(cls, bsteps=BUILD_STEPS, upto=UPTO, itypes=COLLECTORS,
        ...
        defaults = dict(
            ...
            driver=env.driver,
            itype=env.itype,
            compressor=env.compressor.triple,
            ignore_owner=False,
            force=False,
